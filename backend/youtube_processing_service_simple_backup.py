...
class YouTubeProcessingService:
    ...
    async def process_youtube_url(self, youtube_url: str, task_id: str, summary_ratio: float = 0.5, download_video: bool = False):
        ...
        try:
            ...
            # 3단계: 자막이 없으면 오디오 다운로드
            if not self.tasks[task_id]["has_subtitles"]:
                await self._download_audio(task_id, youtube_url, download_video)
    ...
    async def _try_extract_subtitles(self, task_id: str):
        ...
        try:
            ...
            if transcript and transcript.strip():
                ...
                print("자막을 찾을 수 없어 오디오 다운로드로 진행합니다.")
                
        except Exception as e:
            print(f"자막 추출 중 오류 발생: {e}")
            # 자막 추출 실패는 치명적이지 않음 - 오디오 다운로드로 계속 진행
            self.tasks[task_id]["has_subtitles"] = False
            self.tasks[task_id]["progress"] = 40
    
    async def _download_audio(self, task_id: str, youtube_url: str, download_video: bool = False):
        """오디오 다운로드"""
        try:
            ...
            if not file_path:
                error_msg = "YouTube 동영상 다운로드에 실패했습니다. 이 동영상은 다운로드가 제한되었거나 개인 설정으로 인해 접근할 수 없을 수 있습니다."
                print(error_msg)
                raise Exception(error_msg)
            
            self.tasks[task_id]["file_path"] = file_path
            self.tasks[task_id]["progress"] = 60
            print(f"오디오 다운로드 성공: {file_path}")
            
        except Exception as e:
            error_msg = f"오디오 다운로드 실패: {str(e)}"
            print(error_msg)
